...
def run():
    ...
    for twitter_user in users:
        ...
        start_time = datetime.now(tz=timezone.utc) - timedelta(hours= int(twitter_user.hours_auto_rt))
        ...
        end_time = (datetime.now(tz=timezone.utc) - timedelta(hours= int(twitter_user.hours_auto_rt))).replace(microsecond=0).isoformat('T')+ "Z"
