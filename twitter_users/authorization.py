...
def create_update_user_from_twitter_dm(twitter_user_new):
    twitter_user = TwitterUser.objects.filter(twitter_id=twitter_user_new.twitter_id).first()
    if twitter_user is None:
        user = User.objects.filter(username=twitter_user_new.screen_name).first()
        if user is None:
        ...
        twitter_user = TwitterUser(twitter_id=twitter_user_new.twitter_id,
                                   ...
                                   profile_image_url=twitter_user_new.profile_image_url)
        twitter_user.user = user
        twitter_user.twitter_dm_oauth_token = twitter_user_new.twitter_dm_oauth_token
        twitter_user.save()
        return user, twitter_user
    else:
        twitter_user.twitter_dm_oauth_token = twitter_user_new.twitter_dm_oauth_token
...
def dm_check_token_still_valid(twitter_user):
    twitter_api = TwitterAPIDM()
    info = twitter_api.get_me(twitter_user.twitter_dm_oauth_token.oauth_token,
                              twitter_user.twitter_dm_oauth_token.oauth_token_secret)
    return info
